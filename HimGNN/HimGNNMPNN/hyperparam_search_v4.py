import os
os.environ['DGL_HOME'] = '/vols/teaching/msc-projects/2024-2025/aanaidu/.dgl_cache'
...
def main_single(params, args, graph_cache):
    ...
    for fold_id in range(args.folds):
        ...
        val_score_list.append(best_val_score)
    ...
    best_val_score = np.mean(val_score_list)
    best_test_score = np.mean(test_score_list)
    print("AUROC:\n")
    print(max_score_list)
    print(np.mean(test_score_list),'+-',np.std(test_score_list))
    print("AUPRC:\n")
    print(np.mean(max_aupr_list),'+-',np.std(max_aupr_list))
    if task_type == "classification":
        best_val_score = -best_val_score
        best_test_score = -best_test_score
    print("Val score:", best_val_score, " with test_score ", best_test_score)
    global overall_best_val
    global overall_best_test
    if overall_best_val > best_val_score:
        overall_best_val = best_val_score
        overall_best_test = test_score_list
...
if __name__ == '__main__':
    ...
    space = {
    'learning_rate': hp.choice('learning_rate', [0.0001, 0.0005, 0.001, 0.005, 0.1]),
    'drop_rate': hp.choice('drop_rate', [0.1, 0.2, 0.3, 0.5]),
    'batch_size': hp.choice('batch_size', [16, 32, 64]),
    'hid_dim': hp.choice('hid_dim', [64, 96, 128]),
    'dist': hp.choice('dist', [0.01, 0.05, 0.1, 0.2, 0.5, 1]),
    'heads': hp.choice('heads', [1, 2, 4, 8]),

    'gating_func' : hp.choice('gating_func', ['Softmax', 'Sigmoid', "Identity"]),
    'agg_op' : hp.choice('agg_op', ['max', 'mean', 'sum']),
    'num_neurons' : hp.choice('num_neurons', [[256], [256, 256], [512]]),
    'step' : hp.choice('step', [2, 3, 4, 5, 6])
    }

    param_mappings = {
            'batch_size': [16, 32, 64],
            'hid_dim': [64, 96, 128, 256],
            'drop_rate': [0.02, 0.05, 0.1, 0.2, 0.4],
            'learning_rate': [0.0001, 0.0005, 0.001, 0.005, 0.1], 
            'heads': [1, 2, 4, 8],
            'dist' : [0.01, 0.05, 0.1, 0.2, 0.5, 1],
            'gating_func' : ['Softmax', 'Sigmoid', "Identity"],
            'agg_op' : ['max', 'mean', 'sum'],
            'num_neurons' : [[256], [256, 256], [512]],
            'step' : [2, 3, 4, 5, 6]
            }
    ...
    best = fmin(
        fn=lambda params: main_single(params, args, graph_cache),
        space=space,
        algo=tpe.suggest,
        max_evals= max_eval,  # Number of trials
        trials=trials,
        verbose=True,
        early_stop_fn=no_progress_loss(int(max_eval/2))
    )
    best_values = {k: param_mappings[k][v] if k in param_mappings else v for k, v in best.items()}
    ys = [t['result']['loss'] for t in trials.trials]
    print('Dataset {} Hyperopt Results: {}'.format(args.dataset, ys))
    print('Dataset {} Best Params: {}'.format(args.dataset, best_values))
    print('Dataset {} Best Perform: {}'.format(args.dataset, np.min(ys)))
