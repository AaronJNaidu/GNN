...
class Kno(nn.Module):
    def __init__(self, task='reg', tasks=1, attn_head=4, output_dim=128, d_k=64, attn_layers=4, D=16, dropout=0.1, useedge=False, device='cuda:0'):
        ...
        self.act = create_ffn(task, tasks, output_dim + self.global_dim, dropout)
        self.reset_params()
    ...
    def forward(self, data):
        ...
        #print(f"mol_features shape: {data.mol_features.shape}")
        #print(f"x_p shape: {x_p.shape}")

        mol_features = data.mol_features.to(self.device)
        x_p = torch.cat([x_p, mol_features], dim=1) 
