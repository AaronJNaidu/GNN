...
overall_best_val = 1000
overall_best_test = 1000
best_model_paths = None
all_model_files_this_run = []

#def log_relation_weights(model, logger, fold_id=None):
#    label = f" (Fold {fold_id})" if fold_id is not None else ""
#    logger.info(f"Learned Knowledge Attention Weights{label}:")
#    for name, param in model.emb.adj_weight.items():
...
def main(tasks, task, dataset, device, train_epoch, seed, fold, batch_size, rate, split, modelpath, logger, lr, attn_head, output_dim, attn_layers, dropout, mean, stds, D, useedge, met, savem):
    ...
    best_model_files = []
    global all_model_files_this_run
    ...
    if task == 'clas':
        ...
        for fol in range(fold):
            ...
            train_loader, valid_loader, test_loader = load_data(dataset, batch_size, 0, fol, 'clintox_red')
        ...
        return fold_result, best_model_files
        ...
        for fol in range(fold):
            ...
            train_loader, valid_loader, test_loader = load_data(dataset, batch_size, 0, fol, 'esol')
            logger.info('Dataset: {}  Fold: {:<4d}'.format(moldata, fol))
...
def psearch(params):
    ...
    fold_result, saved_model_paths = main(params['tasks'],params['task'],params['moldata'],params['device'],params['train_epoch'],params['seed'],params['fold'],params['batch_size'],params['rate'],params['split'],params['modelpath'],params['logger'],params['lr'],params['attn_head'],params['output_dim'],params['attn_layers'],params['dropout'],params['mean'], params['std'], params['D'], params['useedge'], params['metric'], True)
    if task == 'reg':
        valid_res = np.mean(fold_result[0])
        test_res = np.mean(fold_result[1])
    else:
        valid_res = -np.mean(fold_result[0])
        test_res = -np.mean(fold_result[1])
    print("Validation score:,", np.mean(fold_result[0]), "     Test score:", np.mean(fold_result[1]))
    global overall_best_val
    global overall_best_test
    if overall_best_val > valid_res:
        overall_best_val = valid_res
        overall_best_test = fold_result[1]
        global best_model_paths
        best_model_paths = saved_model_paths
    return valid_res
...
if __name__ == '__main__':
    ...
    if args.mode == 'search':
        ...
        print("Overall best val:", overall_best_val)
        print("Overall best test:", overall_best_test)
        print("Overall best test mean:", np.mean(overall_best_test))
        print("Overall best test std dev:", np.std(overall_best_test))

        weight_sums = None
        n = len(best_model_paths)

        if best_model_paths is not None:
            logger.info("=== Loading and inspecting best models from each fold ===")
            print(best_model_paths)
            for best_model_path in best_model_paths:
                logger.info("Loading overall best model from: " + best_model_path)
                model = Kno(task, numtasks, attn_head, best_values['output_dim'], round(best_values['output_dim']/attn_head), best_values['attn_layers'], best_values['D'], best_values['dropout'], useedge, device).to(device)
                model.load_state_dict(torch.load(best_model_path))
                model.eval()

                if weight_sums is None:
                    weight_sums = {k: 0.0 for k in model.emb.adj_weight.keys()}

                logger.info("Learned Knowledge Attention Weights:")
                for name, param in model.emb.adj_weight.items():
                    logger.info(f"{name}: {param.item():.4f}")
                    weight_sums[name] += param.item()

        logger.info("=== Averaged Knowledge Attention Weights across folds ===")
        for name, total in weight_sums.items():
            logger.info(f"{name}: {total / n:.4f}")

        if 'all_model_files_this_run' in globals() and 'best_model_paths' in globals():
            logger.info("Cleaning up model files from this run...")
            for path in all_model_files_this_run:
                if path not in best_model_paths:
                    try:
                        os.remove(path)
                        logger.info(f"Deleted unused model file: {path}")
        ...
        fold_result, saved_model_path = main(numtasks, task, moldata, device, args.train_epoch, args.seed, args.fold, args.batch_size, rate, args.split, modelpath, logger, args.lr, attn_head, args.output_dim, args.attn_layers, args.dropout, mean, std, args.D, useedge, args.metric, True)
    elif args.mode == 'test':
