...
class Embed(nn.Module):
    def __init__(self, attn_head=4, output_dim=128, d_k=64, attn_layers=4, dropout=0.1, useedge=False, device='cuda:0'):
        ...
        self.adj_weight = nn.ParameterDict({
            'bond_single': nn.Parameter(torch.tensor(0.8)),
            'bond_single_conj': nn.Parameter(torch.tensor(0.825)),
            'bond_double': nn.Parameter(torch.tensor(0.9)),
            'bond_triple': nn.Parameter(torch.tensor(1.0)),
            'bond_aromatic': nn.Parameter(torch.tensor(0.85)),
            'bond_conjugated': nn.Parameter(torch.tensor(0.825)),
            'bond_distant': nn.Parameter(torch.tensor(0.2)),  # very weak attention
        })

    def forward(self, x, edge_index, edge_attr, batch, leng, adj, dis, ring_masm, arom_masm, alip_masm, hete_mask, adj_label_list=None, local_mask_list=None, coj_mask_list=None):
        if self.edge:
        ...
        weight_list = torch.stack([
            torch.tensor(0.0, device=self.device),  # no relation
            self.adj_weight['bond_single'],
            self.adj_weight['bond_single_conj'],
            self.adj_weight['bond_double'],
            self.adj_weight['bond_triple'],
            self.adj_weight['bond_aromatic'],
            self.adj_weight['bond_conjugated'],
            self.adj_weight['bond_distant']
        ])

        for i, l in enumerate(leng):
            L = int(l[0])
            adj_labels = adj_label_list[i]          # numpy array (L,L)
            localmask_np = local_mask_list[i]       # numpy array (L,L)
            cojmask_np = coj_mask_list[i]           # numpy array (L,L)

            adj_labels_t = torch.from_numpy(adj_labels).long()
            localmask = torch.from_numpy(localmask_np).float()
            cojmask = torch.from_numpy(cojmask_np).float()

            adj_tensor = weight_list[adj_labels_t] 
            ...
            matrix_pad[i, :int(l[0]), :int(l[0])] = adj_tensor
...
class Kno(nn.Module):
    ...
    def forward(self, data):
        ...
        x_batch = self.emb(x, edge_index, edge_attr, batch, leng, adj, dis, ring_masm, arom_masm, alip_masm, hete_mask,
                           data.adj_label, data.local_mask, data.coj_mask)
