...
def smiletopyg(smi):
    ...
    c = []
    adj_order_matrix = np.empty((c_size, c_size), dtype=object)
    adj_order_matrix[:] = None
    ...
    for bond in mol.GetBonds():
        ...
        if bond_type == Chem.rdchem.BondType.SINGLE:
            bond_class = 'bond_single_conj' if bond_conj else 'bond_single'
        elif bond_type == Chem.rdchem.BondType.DOUBLE:
            bond_class = 'bond_double'
        elif bond_type == Chem.rdchem.BondType.TRIPLE:
            bond_class = 'bond_triple'
        elif bond_type == Chem.rdchem.BondType.AROMATIC:
            bond_class = 'bond_aromatic'
        else:
            bond_class = 'bond_unknown'

        # Save string label into adj_order_matrix temporarily
        adj_order_matrix[a1, a2] = bond_class
        adj_order_matrix[a2, a1] = bond_class
    ...
    for i in range(c_size):
        for j in range(i,c_size):
            if adj_order_matrix[i, j] is None and i != j:
                conj = False
                ...
                for path in paths:
                    if set(path) < set(c):
                        ...
                        break
                if conj:
                    adj_order_matrix[i, j] = 'bond_conjugated'
                    adj_order_matrix[j, i] = 'bond_conjugated'
                else:
                    path = paths[0]
                    adj_order_matrix[i, j] = 'bond_distant'
                    adj_order_matrix[j, i] = 'bond_distant'
